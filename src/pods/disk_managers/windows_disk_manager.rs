...
impl DiskManager for WindowsDiskManager {
    fn new_file(&self, path: &WhPath, _permissions: u16) -> io::Result<()> {
        std::fs::File::create(&self.mount_point.join(path)).inspect_err(|e| log::error!("WDM::new_file Error: {e}"))?;
        Ok(())
    }

    fn remove_file(&self, path: &WhPath) -> io::Result<()> {
        std::fs::remove_file(&self.mount_point.join(path)).inspect_err(|e| log::error!("WDM::remove_file Error: {e}"))
    }

    fn remove_dir(&self, path: &WhPath) -> io::Result<()> {
        std::fs::remove_dir(&self.mount_point.join(path)).inspect_err(|e| log::error!("WDM::remove_dir Error: {e}"))
    }

    fn write_file(&self, path: &WhPath, binary: &[u8], offset: usize) -> io::Result<usize> {
        std::fs::File::create(&self.mount_point.join(path))?.seek_write(binary, offset as u64).inspect_err(|e| log::error!("WDM::write_file Error: {e}"))
    }
    ...
    fn read_file(&self, path: &WhPath, offset: usize, buf: &mut [u8]) -> io::Result<usize> {
        std::fs::File::open(&self.mount_point.join(path))?.seek_read(buf, offset as u64).inspect_err(|e| log::error!("WDM::read_file Error: {e}"))
    }

    fn new_dir(&self, path: &WhPath, _permissions: u16) -> io::Result<()> {
        std::fs::create_dir(&self.mount_point.join(path)).inspect_err(|e| log::error!("WDM::new_dir Error: {e}"))
    }
