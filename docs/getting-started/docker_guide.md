...
We provide an official Docker image hosted on the GitHub Container Registry (GHCR).

You can pull it directly:

```sh
docker pull ghcr.io/agartha-software/wormhole:latest
```

To run a simple wormhole node:
...
docker run -d \
  ...
  ghcr.io/agartha-software/wormhole:latest
```

> [!IMPORTANT]
> The `--device /dev/fuse`, `--cap-add SYS_ADMIN`, and `--security-opt apparmor:unconfined` options are required.
> Wormhole uses FUSE to create its virtual filesystem, which requires elevated privileges within the container.
