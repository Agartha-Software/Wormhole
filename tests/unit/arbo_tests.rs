...
fn test_inserting_and_retreiving_files() {
    ...
    assert!(
        arbo.add_inode_from_parameters("file1".to_owned(), 10, ROOT, FsEntry::File(Vec::new()), 0o777)
            .is_ok(),
        "can't add file1 in / folder"
    );
    assert!(
        arbo.add_inode_from_parameters("file2".to_owned(), 11, ROOT, FsEntry::File(Vec::new()), 0o777)
            .is_ok(),
